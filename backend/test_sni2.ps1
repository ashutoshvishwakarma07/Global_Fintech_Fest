...
try {
    ...
    $uBase64 = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes('visiting.cardapp@qualtechedge.in'))
    $sslWriter.WriteLine($uBase64)
    Write-Host "User resp: $($sslReader.ReadLine())"
    
    # Clean app password
    $pBase64 = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes('setodqatwfoijovo'))
